...
\begin{frame}{MIP -- variáveis e objectivo}
  \[\small
    x_{cdr}\in\{0,1\}\;\forall c,\;d\in D_c,\;r
    \qquad
    z_c\in[0,1]\;\forall c:\;p_c\ne4
  \]
  $z_c$ relaxado para contínuo; C3 força-o a $\{0,1\}$ no óptimo.
  \[\small
    \min\;\sum_{c:\,dd_c\ge0}\sum_{d,r}[dd_c+d]\,x_{cdr}
    +\sum_{c:\,dd_c<0}\sum_{d,r}[dd_c+\alpha d]\,x_{cdr}
    +\sum_{c:\,p_c\ne4}w_c\,z_c
  \]
  Mesma estrutura de três termos que os Termos 1--3 do CP-SAT; o mesmo
  \texttt{penalty.py} calcula $w_c$. Sem Termo 4.
  \bk
\end{frame}
